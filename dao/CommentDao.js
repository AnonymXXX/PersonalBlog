...
function insertComment(blogId, parent, parentName, userName, comments, email, ctime, utime, success) {
    var insertSql = "insert into comments (`blog_id`, `parent`, `parent_name`, `user_name`, `content`, `email`, `ctime`, `utime`) values(?, ?, ?, ?, ?, ?, ?, ?);";
...
function queryNewComments(size, success) {
    var querySql = "select * from comments where parent = -1 or parent = -2 order by id desc limit ?;";
    var params = [size];
